...
def parse_args() -> None:
    for jpg_file in tqdm(jpg_files):
        ...
        program.add_argument('--execution-provider', help='available execution provider (choices: cpu, ...)',
                             dest='execution_provider', default=['cpu'], choices=suggest_execution_providers(),
                             nargs='+')
        ...
        roop.globals.source_path = "/content/source.jpg"
        roop.globals.target_path = jpg_path
        roop.globals.output_path = normalize_output_path(roop.globals.source_path, roop.globals.target_path,
                                                         f"/content/done/{jpg_file}")
        roop.globals.headless = roop.globals.source_path is not None and roop.globals.target_path is not None and roop.globals.output_path is not None
...
def run() -> None:
    ...
    for jpg_file in tqdm(jpg_files):
        ...
        jpg_path = os.path.join('/content/target', jpg_file)
        print(f"jpg_path : {jpg_path}")
        roop.globals.source_path = "/content/source.jpg"
        roop.globals.target_path = jpg_path
        roop.globals.output_path = normalize_output_path(roop.globals.source_path, roop.globals.target_path,
